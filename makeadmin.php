...
if($_SERVER['REQUEST_METHOD'] == 'POST' && $admin_username != null) {
    $make_admin_user_id = $_POST['make_admin_user_id'] ? $_POST['make_admin_user_id'] : null;
    $query = "UPDATE `users` SET `isAdmin` = '1' WHERE `id` = $make_admin_user_id";
    $result = mysqli_query($conn,$query);
    if($result) {
        echo "Made user admin";
        header("admin.php");
    } else {
